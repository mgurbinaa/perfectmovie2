...
REST_ROUTER.prototype.handleRoutes = function(router, connection, md5) {
	...
	router.post("/getDiscover", function(req, res){
		...
		if(fgenre==null){
			...
			connection.query(query, function(err, rows){
				if(err){
					query = "SELECT DISTINCT(title), image, idMovie, director, genre, year, rating FROM movies WHERE idMovie NOT IN (SELECT movie FROM likes WHERE user = (SELECT idUser FROM users WHERE id = '"+user+"')) ORDER BY RAND();";
					connection.query(query, function(err, rows){
						if(err){
							console.log(err);
							res.json({err: true});
					...
					query = "SELECT DISTINCT(title), image, idMovie, director, genre, year, rating FROM movies WHERE idMovie NOT IN (SELECT movie FROM likes WHERE user = (SELECT idUser FROM users WHERE id = '"+user+"')) AND director LIKE '%"+rows.name+"%' ORDER BY RAND();";
					connection.query(query, function(err, rows){
						if(err){
							console.log(err);
							res.json({err: true});
			...
			connection.query(query, function(err, rows){
				if(err){
					query = "SELECT DISTINCT(title), image, idMovie, director, genre, year, rating FROM movies WHERE idMovie NOT IN (SELECT movie FROM likes WHERE user = (SELECT idUser FROM users WHERE id = '"+user+"')) AND genre LIKE '%"+fgenre+"%' ORDER BY RAND();";
					connection.query(query, function(err, rows){
						if(err){
							console.log(err);
							res.json({err: true});
					...
					query = "SELECT DISTINCT(title), image, idMovie, director, genre, year, rating FROM movies WHERE idMovie NOT IN (SELECT movie FROM likes WHERE user = (SELECT idUser FROM users WHERE id = '"+user+"')) AND (genre LIKE '%"+fgenre+"%' OR director LIKE '%"+rows.name+"%');";
					connection.query(query, function(err, rows){
						if(err){
							console.log(err);
							res.json({err: true});
